...
if (global.search.term !== "" && global.search.term !== null) {
  const {results, total_pages, page, total_results} = await searchAPIDate();
...
{
  global.search.page++;
  const { results, total_pages } = await searchAPIDate();
  displaySearchResults(results);
});
 ...
 document.querySelector("#next-bottom").addEventListener("click", async () => 
{
  global.search.page++;
  const { results, total_pages } = await searchAPIDate();
  displaySearchResults(results);
 ...
 // Prev page
 document.querySelector("#prev-bottom").addEventListener("click", async () => 
 {
   global.search.page--;
   const { results, total_pages } = await searchAPIDate();
   displaySearchResults(results);
